...
different_manu <- distinct(manufacturers, make)

# Filter the data set for vehicles manufactured in 1997
vehicles_1997 <- filter(vehicles, year == "1997")

# Arrange the 1997 cars by highway (`hwy`) gas milage
hwy_arranged <- arrange(vehicles_1997, hwy)

# Mutate the 1997 cars data frame to add a column `average` that has the average
# gas milage (between city and highway mpg) for each car
avg_gas_1997 <- mutate(vehicles, average = (hwy + cty) / 2)
...
vehicle_id <- select(worst_vehicle, id)
...
most_miles <- function(year_choice, make_choice) {
  ...
  best <- filter(models, hwy == max(hwy))
  best_models <- select(best, model)
  print(best_models)
}

# What was the most efficient Honda model of 1995?
most_miles("1995", "Honda")
